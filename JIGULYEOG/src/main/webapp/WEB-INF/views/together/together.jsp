       ...
       <form action="together.do" method="get" id="searchForm" style="display:inline-block;">
          ...
          <div class="dropdown" style="float: right;">
            ...
            <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
              <a class="dropdown-item" href="together.do?category=1">모금</a>
              <a class="dropdown-item" href="together.do?category=2">나눔</a>
              <a class="dropdown-item" href="together.do?category=3">봉사</a>
            </div>
			                        ...
			                        <div class="card fundraise-item mb-5">
			                            <div class="card-body">
			                              ...
			                              <span class="donation-time mb-3 d-block">
			                              	<b>${dto.user_id }</b> 
       										<fmt:formatDate value="${dto.tog_dead}" pattern="yyyy/MM/dd"/>
       									</span>
			                            </div>
			                        </div>
			                    </div>			
        ...
        <nav aria-label="Page navigation">
          <ul class="pagination">
          	<c:if test="${pageMaker.prev }">
	            <li class="page-item">
	              <a class="page-link" href="together.do?page=${pageMaker.startPage - 1 }&category=${category}&keyword=${keyword }" aria-label="Previous">
          	...
          	<c:forEach begin="${pageMaker.startPage }" end="${pageMaker.endPage }" var="idx">
          		<c:choose>
          			<c:when test="${idx eq page }">
			            <li class="page-item active"><a class="page-link" href="together.do?page=${idx }&category=${category}&keyword=${keyword }">${idx }</a></li>
          			</c:when>
          			<c:otherwise>          				
			            <li class="page-item"><a class="page-link" href="together.do?page=${idx }&category=${category}&keyword=${keyword }">${idx }</a></li>
          			</c:otherwise>
          		</c:choose>          		
          	</c:forEach>
            ...
            <c:if test="${pageMaker.next && pageMaker.endPage>0 }">
	            <li class="page-item">
	              <a class="page-link" href="together.do?page=${pageMaker.endPage + 1 }&category=${category}&keyword=${keyword }" aria-label="Next">
